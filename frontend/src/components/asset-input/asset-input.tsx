...
import {UnitQuantityInput} from './unit-quantity-input'
import {UnitSelect} from './unit-select'

// buffer that is kept in the wallet to pay for tx fee
const LOVELACE_BUFFER = 2_000_000n
  ...
  const handleMaxButtonClick = () => {
    if (value.unit && selectedUnitBalance) {
      const maxQuantity = isLovelaceUnit(value.unit)
        ? bigIntMax(selectedUnitBalance - LOVELACE_BUFFER, 0n)
        : selectedUnitBalance
      ...
      <div className="flex flex-row items-stretch gap-2">
        ...
        <div className="flex-1">
          <UnitQuantityInput
            ...
            unit={value.unit}
            className="h-full border-none bg-transparent text-right text-3xl md:text-3xl"
            disabled={disabled}
          />
        </div>
      </div>
              ...
              <div className="flex flex-row items-center gap-2 text-gray-400">
                ...
                {balanceState === 'loading' ? (
                  ...
                  <p className="text-sm">
                    <AssetQuantity
                      unit={value.unit}
                      quantity={selectedUnitBalance ?? 0n}
                      showTicker={false}
                    />
                  </p>
                ) : (
